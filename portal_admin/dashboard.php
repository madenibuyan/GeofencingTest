...
<head>
	...
	<title>Admin Dashboard :: Attendance System using Geofencing</title>
...
<body>
    ...
    <div class="wrapper theme-6-active pimary-color-green">
		...
		<div class="fixed-sidebar-left">
			<ul class="nav navbar-nav side-nav nicescroll-bar">
				...
				<li>
					<a href="dashboard.php" class="active"><div class="pull-left"><i class="fa fa-dashboard mr-20"></i><span class="right-nav-text">Dashboard</span></div>
				...
				<div class="row">
					...
					<div class="panel panel-inverse card-view">
							<div class="panel-heading">
									<div class="pull-left">
										<h6 class="panel-title" style="color:black; font-weight:bold; font-size:15px; margin-top:-10px; margin-bottom:-10px;">ADD STUDENT</h6>
									</div>
									</div>
							<div class="panel-wrapper collapse in">
					...
					<div style="magin-bottom:5px; margin-top:5px;"></div>
									...
									<input type="hidden" name="add_student" value="dosumu">
												<!-- surname 	othernames 	matric_no 	department 	level  -->
									<div class="form-group"> 
								<label class="control-label mb-5" for="email_de">Matric No</label>
						<input class="form-control" name="matric_no" required >
												</div>
									<div class="form-group"> 
								<label class="control-label mb-5" for="email_de">Surname</label>
						<input class="form-control" name="surname" required >
												</div>
									<div class="form-group">
								<label class="control-label mb-5" for="email_de">Othernames</label>
						<input class="form-control" name="othernames" required >
												</div>
												
									<div class="form-group">
								<label class="control-label mb-5" for="email_de">Department</label>
						<input class="form-control" name="department" required >
												</div>
												
												<div class="form-group">
                             <label class="control-label mb-5" for="email_de">Level</label>
												<select class="form-control" name="level" id="num_semester" required >
												<option value="">Select</option>
												<?php
												for($i=1; $i<=9; $i++){
													echo '<option value="'.$i.'00">'.$i.'00L</option>';
												}
												?>
												</select>
                                                </div>
												...
												<div class="msg"></div>
													<div class="form-group">
									<button class="btn btn-primary btn-sm mb-5 pull-right" type="submit">Add Student</button>
					...
					//	surname 	othernames 	matric_no 	department 	level
	$mom=mysqli_query($con,"select * from `student` order by surname asc,department asc,level asc");
	...
	$no=$no+1;		
											...
											echo '<tr id="'.$cel['id'].'">
											<td>'.$no.'.</td>
											<td>'.ucfirst($cel['surname']).'</td>
											<td>'.ucwords($cel['othernames']).'</td>
											<td>'.strtoupper($cel['matric_no']).'</td>
											<td>'.strtoupper($cel['department']).'</td>
											<td>'.strtoupper($cel['level']).'</td>
											
											<td><a href="javascript:void(0)" id="'.$cel['id'].'" title="Delete" class="delete_student text-danger" style="font-size:19px;"><i class="fa fa-trash"></i></a></td>
											</tr>';
